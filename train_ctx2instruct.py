...
PROMPT_DICT = {
    "prompt_input": (
        "아래는 질문 혹은 명령어에 따른 요청을 입력된 문서를 기반으로 적절히 완료하는 응답입니다.\n\n"
        "아래 입력된 문서와 작성된 답변에 대응되는 질문 혹은 명령어를 작성하세요.\n\n"
        "문서: {input}\n답변: {output}\n질문:"
    ),
    
    "prompt_no_input": (
        "아래는 질문 혹은 명령어에 따른 요청을 적절히 완료하는 응답입니다.\n\n"
        "아래 작성된 답변에 대응되는 질문 혹은 명령어를 작성하세요.\n\n"
        "답변: {output}\n질문:"
    ),
}
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        if os.path.exists(data_cache_name):
            logging.warning("Loading cached data...")
            ...
            targets = [f"{example['instruction']}{tokenizer.eos_token}" for example in list_data_dict]
            ...
            self.input_ids = data_dict["input_ids"]
            self.labels = data_dict["labels"]
            logging.warning("Caching tokenized inputs...")
...
def train():
    ...
    if model_args.tokenizer_name:
        tokenizer = transformers.AutoTokenizer.from_pretrained(model_args.tokenizer_name, **tokenizer_kwargs)
    elif model_args.model_name_or_path:
