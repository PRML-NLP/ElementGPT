...
class GPTchatClass:
    def __init__(
            ...
            role_msg: str = """Your are a binary answering assistant. 
                    If the given '예측' is even remotely close to '정답'
                    Generate a single output 'True' 
                    and if '예측' in any way signifies '정답'
                    Generate a single output 'True'
                    and if the given '예측' is absolutely ludacris to '정답',
...
def completion_loop(args):
    ...
    # transformers.utils.move_cache(cache_dir=args.cache)
    ...
    ref = df['해설'].reset_index()
    datasets = pd.concat([valid["문제"], label["정답"], ref['해설']], axis=1)
    GPT = GPTchatClass(
    ...
    data_list = []
    for q, t, c in zip(tqdm(datasets['문제']), datasets['정답'], datasets['해설']):
        conv = get_conv_template(args.prompt) # 추론을 위한 프롬프트
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subject", type=str, default="./demos/society_manual.json", help="problem_file", choices=['./demos/science_manual.json', './demos/society_manual.json', './demos/korean_manual.json', './demos/math_manual.json'])
    parser.add_argument("--prompt", type=str, default="elementgpt_for_society_short", help="Instruction and sample exercise", choices=['elementgpt_for_science_short', 'elementgpt_for_society_short', 'elementgpt_for_korean_short', 'elementgpt_for_society_short'])
    parser.add_argument("--output_dir", type=str, default="./outputs/society_short_answer.json", help="answer_sheet")
