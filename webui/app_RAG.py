...
from completion import generate_stream
...
MODEL_NAME = "./experiments/poly5.8b-DAPT2INST_wo_bank_b120"
ADAPTER_NAME = None
DEVICE_ID = 0
...
def predict(message, history, sys_prompt, temp, rep):
    ...
    docs = langchain(message)
    sentence = ""
    for doc in docs:
        split_text = str(doc).split("page_content=")[1]
        page_content = split_text.split("metadata=")[0]
        sentence = sentence + "\n" + page_content
    input_prompt = "reference" + sentence + "\n\n" + input_prompt + "\n\n" + "querry: " + message
    gen_params = {
        "model": MODEL_NAME,
        "prompt": input_prompt,
    ...
    with open(os.path.join(LOG_DIR, end_time + ".json"), "w") as fout:
        json.dump({"prompt": input_prompt, "instruction": message, "response": partial_message}, fout,
